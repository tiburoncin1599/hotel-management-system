...
import type { RoomWithRelations, PaginatedResult } from '../types';
import { useState } from 'react';
import { DeleteRoomDialog } from './delete-dialog';

interface RoomsTableProps {
  data: PaginatedResult<RoomWithRelations>;
...
export function RoomsTable({ data, statuses, types }: RoomsTableProps) {
  ...
  function setParam(key: string, value: string) {
    ...
    params.set('page', '1');
    router.push(`/dashboard/habitaciones?${params.toString()}`);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          href="/dashboard/habitaciones/nuevo"
                ...
                <tr key={room.id} className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-right text-sm">
                    <div className="flex items-center justify-end gap-1">
                      <Link
                        href={`/dashboard/habitaciones/${room.id}`}
      ...
      <DeleteRoomDialog roomId={deleteId} onClose={() => setDeleteId(null)} />
    </div>
  );
}
